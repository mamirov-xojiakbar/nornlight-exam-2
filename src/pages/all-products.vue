<template>
  <div id="mt" class="mb-[100px] container mx-auto px-4">
    <p id="text" class="font-bold text-2xl md:text-4xl leading-tight mb-10">Популярные товары</p>

    <Range :maxValue="10000" :minValue="0" @update="updateRange" />
...
const range = ref({ min: 0, max: 10000 });
...
const filteredItems = computed(() => {
  return items.value.filter(item => item.price >= range.value.min && item.price <= range.value.max);
});
